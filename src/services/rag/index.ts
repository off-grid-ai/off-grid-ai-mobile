import { ragDatabase, ExportedDocument } from './database';
...
export type { RagDocument, RagSearchResult, ExportedDocument, ExportedChunk } from './database';
...
class RagService {
  ...
  /** Export a project's full knowledge base (docs + chunks + embeddings) for backup. */
  async exportProjectDocuments(projectId: string): Promise<ExportedDocument[]> {
    await this.ensureReady();
    return ragDatabase.exportProjectDocuments(projectId);
  }

  /**
   * Restore a project's knowledge base from a backup. Ensures the embedding model
   * is loaded FIRST (the caller-requested guard) so any chunk without a portable
   * embedding can be regenerated. Stored vectors are reused directly when they
   * match the current embedding dimension; mismatched/absent vectors are dropped
   * and regenerated by backfill. Import is additive: a document whose name already
   * exists in the project is skipped. Returns how many docs were imported/skipped.
   */
  async importProjectDocuments(
    projectId: string,
    docs: ExportedDocument[],
  ): Promise<{ imported: number; skipped: number }> {
    await this.ensureReady();
    // Guard: the embedding model must be present/loaded before we import, both to
    // validate dimension and to backfill any chunk we can't reuse a vector for.
    await embeddingService.load();
    ...
    let skipped = 0;
    for (const doc of docs) {
      // Reuse a stored vector only if it matches the current model's dimension;
      // otherwise drop it so backfill regenerates a compatible one.
      const normalized: ExportedDocument = {
        ...doc,
        chunks: doc.chunks.map((c) => ({
          ...c,
          embedding: Array.isArray(c.embedding) && c.embedding.length === dim ? c.embedding : null,
        })),
      };
      if (ragDatabase.importDocument(projectId, normalized)) imported += 1;
      else skipped += 1;
    }
    // Regenerate embeddings for any imported doc that arrived without usable vectors.
    await this.backfillEmbeddings(projectId);

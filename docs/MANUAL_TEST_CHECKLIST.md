...
| T056 | 🔴 P1 | ✅ `errorClearsSpinner.rendered.redflow` (local; remote path still ~) | Drive a generation that errors (e.g. the B9 vision decode fail) | the loading spinner CLEARS + an error bubble renders (RED: session ends reason=error but UI spins forever) | DEV-B13 · BROKEN | |

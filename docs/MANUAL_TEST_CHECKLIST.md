...
| T010 | 🔴 P2 | ❌ DEFERRED (needs RAG UI harness) | Attach a **scanned/image** PDF (pdf fake textLength:0) to a KB | a clear "no text layer / scanned PDF" message renders (RED: vague "could not extract text"). **DEFERRED — no mounted-screen RAG doc-attach harness exists (same gap as T118); needs a project+KB attach-document UI harness. Not a device mismatch** | DEV · 0-text vague | |
| T011 | ✅ P2 | ❌ DEFERRED (needs RAG UI harness) | Attach a **>5MB** PDF to a KB (fake file size >5MB) | "Maximum file size is 5MB" renders + upload rejected. **DEFERRED — needs the same project+KB attach-document UI harness as T010/T118. Guard is service-covered; the mounted-UI gesture needs the RAG harness** | DEV · GATED | |
